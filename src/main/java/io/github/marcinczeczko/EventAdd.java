...
@Named("add")
public class EventAdd implements RequestHandler<APIGatewayProxyRequestEvent, APIGatewayProxyResponseEvent> {
  ...
  public APIGatewayProxyResponseEvent handleRequest(APIGatewayProxyRequestEvent request, final Context context) {
    ...
    try {
      Event event = RequestUtils.fromAddRequest(request);
      event.setHashId();
      service.addItem(event);
      return RequestUtils.successResponse(event);
